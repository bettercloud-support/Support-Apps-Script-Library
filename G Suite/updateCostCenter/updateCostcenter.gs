...
function myFunction() {
  ...
  var input = SpreadsheetApp.getActiveSpreadsheet().getRange("B2:B"+maxRows).getValues(); //Add Cost Center to the B column in the spreadsheet
 
  for (var i = 0; i <users.length;i++) {
 
    var cCenter = input[i].toString();
 
    try {
 
    var User = AdminDirectory.Users.get(users[i]);
      User.organizations[1].costCenter = cCenter;
      AdminDirectory.Users.update(User,users[i]);
    ...
    SpreadsheetApp.getActiveSpreadsheet().getActiveSheet().getRange("C"+(i+2)).setValue("Failed to Update Cost Center");
